...
for i, height in enumerate(np.arange(0.7, 2.0, 0.01)):
    ...
    args = ['./cupgame-nowait-mapgen', N, height, 0.90, 
            x0, size + x0, 
            0., size + 0., tfile]
    ...
    out = cm.jet(Normalize(vmin=0, vmax=np.log(160))(np.log(tt+1)))

    imsave(join(outdir, 'energy_%04d.png' % i), out)
