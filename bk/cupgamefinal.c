...
void build_torus_poly(double *pos, double *vel, 
        double h, double r, double torusx, double torusy, double *poly){
...
void build_plane_poly(double *pos, double *vel, double r, double *poly){
    poly[0] = pos[2]-r; poly[1] = vel[2]; poly[2] = -0.5;
}
...
int distance_to_torus(double *pos, double h, double r, double *cup){
    double dr[3];
    dr[0] = pos[0] - cup[0];
    dr[1] = pos[1] - cup[1];
    dr[2] = pos[2];
    double term1 = (dot(dr,dr) + h*h - r*r);
    double term2 = 4*h*h*(dr[0]*dr[0] + dr[1]*dr[1]);
...
int collides_with_cup(double *pos, double *vel, double h, double r, 
    ...
    */
    ...
    build_torus_poly(tpos, tvel, h, r, cx, cy, poly);
    *tcoll = find_smallest_root(poly, DEG);
    ...
    if (isnan(*tcoll)){
        ...
        *tcoll = find_smallest_root(poly, 2);
        ...
        if ((tpos[0]-cx)*(tpos[0]-cx)+(tpos[1]-cy)*(tpos[1]-cy) > (h+r)*(h+r))
            return 0;
        return 2;
    } 
    return 1;
...
int collision_time(double *pos, double *vel, double h, 
    ...
    /* 
     * This function figures out the next time that a ball hits any cup,
     * if any.  To indicate so, it returns:
     *  0 : There was no collision 
     *  1 : There was a collision and it occured at tcoll
     *  2 : The ball passed through the center of a cup
    */
    int i, result, ncups;
    double cups[MAXCUPS], tpos[3], tvel[3], poly[DEGSIZE];
    memcpy(tpos, pos, sizeof(double)*3);
    memcpy(tvel, vel, sizeof(double)*3);

    // check if are going to hit our neighbors on the way out
    cups_near(pos, cups, &ncups);
    for (i=0; i<ncups; i++){
        result = collides_with_cup(pos, vel, h, r, cups[2*i+0], cups[2*i+1], tcoll);
    ...
    // get close to the region of interest. i.e. bring the
    // ball close to the plane of the cups
    /*build_plane_poly(tpos, tvel, r, poly);
    *tcoll = find_smallest_root(poly, 2);
    position(tpos, tvel, *tcoll, tpos); 
   
    cups_near(tpos, cups, &ncups);
    for (i=0; i<ncups; i++){
        result = collides_with_cup(pos, vel, h, r, cups[2*i+0], cups[2*i+1], tcoll);
        if (result == RESULT_COLLISION || result == RESULT_INCUP){
            cup[0] = cups[2*i+0];
